...
const log = (extended, development) => {
  return async (req, res, next) => {
    ...
    res.on('finish', async () => {
      ...
      const latLong = _.split(req.connection.citylatlong, ',');

      const location = {
        geohash: geohash.encode(latLong[0], latLong[1]),
        city: req.connection.city,
        country: req.connection.country,
        latLong: req.connection.citylatlong,
      };
      ...
      object.path = res.statusCode !== 404 && req.route && req.route.path ? req.route.path : ' ';
      ...
      object.cookies = _.isEmpty(req.cookies) ? ' ' : JSON.stringify(req.cookies);
      object.auth = req.header('Authorization') || req.header('x-access-token') || ' ';
...
const getBasic = (req, res, hostinfo, elapsed) => {
  ...
  console.info(`Content Type: ${req.header('Content-Type') || 'No content type given'}`);
  console.info(`Hostname: ${req.hostname}`);
  console.info(`OS hostname: ${hostinfo.hostname}`);
  console.info(
    `Path & URL: ${(req.route && req.route.path) || 'No Path'} ––– ${req.originalUrl ||
      req.url ||
      'No Url'}`,
  );
};
...
const getParameters = (req, realBody) => {
  ...
  console.info(`Cookies & Storage: ${JSON.stringify(req.cookies) || 'No tasty cookies 🍪'}`);
};
...
const getAuth = req => {
  ...
  console.info(
    `Client: ${req.connection.remoteAddress || req.ip || 'No IP found'} ––– ${req.header(
      'User-Agent',
    )}`,
  );
};
...
const addToObject = ({ name, timing, ...custom } = {}) => {
  ...
  multilog.pushCustomData(databaseMetrics);
};
