...
const influxFunctions = {
      ...
      schema: [
        ...
        },
        {
          ...
          fields: {
            ...
            latLong: Influx.FieldType.STRING,
          ...
          tags: ['path', 'geohash', 'country', 'city', 'latLong'],
        },
      ],
    });
  },
  ...
  writeToDatabase: async (influx, name) => {
        ...
        _.map(multiLogData, object => {
          return {
            tags: {
              path: object.path,
              geohash:
                object.location && object.location.geohash ? object.location.geohash : 'notFound',
              country:
                object.location && object.location.country ? object.location.country : 'notFound',
              city: object.location && object.location.city ? object.location.city : 'notFound',
              latLong:
                object.location && object.location.latLong ? object.location.latLong : 'notFound',
            },
            fields: {
              responseTime: object.responseTime,
              requests: multiLogData.length,
              host: process.env.GAE_APPLICATION
                ? `${process.env.GAE_APPLICATION}-${process.env.GAE_INSTANCE}`
                : object.hostname,
              country:
                object.location && object.location.country ? object.location.country : 'notFound',
              geohash:
    ...
    influx.writeMeasurement('performance', [
      {
        tags: {
          hostname: process.env.GAE_APPLICATION
            ? `${process.env.GAE_APPLICATION}-${process.env.GAE_INSTANCE}`
            : _.get(object, 'hostname', 'not set'),
        },
        fields: {
